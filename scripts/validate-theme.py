...
def main():
    ...
    wallpapers = sorted(p for p in (ROOT / "backgrounds").iterdir() if p.suffix in {".png", ".jpg"})
    assert len(wallpapers) == 4, "exactly four wallpapers are required"
    assert png_size(ROOT / "backgrounds/04-omarchy-wordmark.png") == (3840, 2160)
    assert all(path.stat().st_size > 1000 for path in wallpapers)
